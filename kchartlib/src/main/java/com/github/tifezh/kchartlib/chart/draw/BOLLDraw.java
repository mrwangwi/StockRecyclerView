...
public class BOLLDraw implements IChartDraw<IBOLL> {
    ...
    @Override
    public void drawTranslated1(@Nullable IBOLL lastPoint, @NonNull IBOLL curPoint, float lastX, float curX, @NonNull Canvas canvas, @NonNull BaseKChartView view, int position) {
        view.drawChildLine1(canvas, mUpPaint, lastX, lastPoint.getUp(), curX, curPoint.getUp());
        view.drawChildLine1(canvas, mMbPaint, lastX, lastPoint.getMb(), curX, curPoint.getMb());
        view.drawChildLine1(canvas, mDnPaint, lastX, lastPoint.getDn(), curX, curPoint.getDn());
    }
