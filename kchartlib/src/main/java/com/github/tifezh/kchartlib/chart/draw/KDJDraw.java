...
public class KDJDraw implements IChartDraw<IKDJ> {
    ...
    @Override
    public void drawTranslated1(@Nullable IKDJ lastPoint, @NonNull IKDJ curPoint, float lastX, float curX, @NonNull Canvas canvas, @NonNull BaseKChartView view, int position) {
        view.drawChildLine1(canvas, mKPaint, lastX, lastPoint.getK(), curX, curPoint.getK());
        view.drawChildLine1(canvas, mDPaint, lastX, lastPoint.getD(), curX, curPoint.getD());
        view.drawChildLine1(canvas, mJPaint, lastX, lastPoint.getJ(), curX, curPoint.getJ());
    }
