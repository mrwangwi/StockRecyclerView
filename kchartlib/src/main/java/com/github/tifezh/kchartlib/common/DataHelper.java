...
public class DataHelper {
    ...
    static void calculateRSI(List<KLineEntity> datas) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (i == 0) {
                ...
                rsi1 = rsi1ABSEma != 0 ? (rsi1MaxEma / rsi1ABSEma) * 100 : 100;
                rsi2 = rsi2ABSEma != 0 ? (rsi2MaxEma / rsi2ABSEma) * 100 : 100;
                rsi3 = rsi3ABSEma != 0 ? (rsi3MaxEma / rsi3ABSEma) * 100 : 100;
            }
            point.rsi1 = rsi1;
            point.rsi2 = rsi2;
            point.rsi3 = rsi3;
     ...
     * @param datas
     */
    static void calculateChangePrice(List<KLineEntity> datas) {
        for (int i = 0; i < datas.size(); i++) {
            KLineEntity point = datas.get(i);
            if (i == 0) {
                point.changePrice = "--";
                ...
                float price = datas.get(i).getClosePrice() - datas.get(i - 1).getClosePrice();
                point.changePrice = TextUtil.doubleFormat(price + "");
                point.changeRatio = TextUtil.doubleFormat(price * 100 / datas.get(i - 1).getClosePrice() + "") + "%";
            }
        }
    }
    ...
    static void calculatePrClosePrice(List<KLineEntity> datas) {
        for (int i = 0; i < datas.size(); i++) {
            KLineEntity point = datas.get(i);
            if (i == 0) {
                point.prClosePrice = 0;
            } else {
                point.prClosePrice = datas.get(i - 1).getClosePrice();
            }
        }
    }
...
//            用（DIF-DEA）*2即为MACD柱状图。
     ...
     * @param datas
     */
    static List<KLineEntity> calculate(List<KLineEntity> datas) {
        calculateMA(datas);
        calculateMACD(datas);
        calculateBOLL(datas);
        calculateRSI(datas);
        calculatePrClosePrice(datas);
        calculateChangePrice(datas);
        calculateKDJ(datas);
        calculateVolumeMA(datas);
        calculateBalanceMA(datas);
        return datas;
    }
    ...
    private static void calculateBalanceMA(List<KLineEntity> entries) {
        float balanceMa5 = 0;
        float balanceMa10 = 0;
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            balanceMa5 += entry.getBalance();
            balanceMa10 += entry.getBalance();

            if (i >= 5) {

                balanceMa5 -= entries.get(i - 5).getBalance();
                entry.MA5Balance = (balanceMa5 / 5f);
            } else {

                entry.MA5Balance = (balanceMa5 / (i + 1f));
            }

            if (i >= 10) {
                balanceMa10 -= entries.get(i - 10).getBalance();
                entry.MA10Balance = (balanceMa10 / 10f);
            } else {
                entry.MA10Balance = (balanceMa10 / (i + 1f));
            }
        }
    }
}
